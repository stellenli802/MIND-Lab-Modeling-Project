...
setwd("G:/projects/tpgarcia/enroll-hd/02-Kyle-Comparing-Time-To-Diagnosis-Models")
...
ENROLL <- readRDS("enrollhd_baseline_survival_data.rds")
...
# PIN <- (PI_HD - 883)/1044
# CAP_data$CAPs <- CAP_data$CAP / ( (CAP$coefficients[1]) - log(5) / ( -1 * CAP$coefficients[2] ) ) # requires CAP model below to be run for this line
```
...
pec_obj <- getBrierScore(fit = MRS_clean,
                         data = MRS_data_clean,
                         formula = Surv(W, delta) ~ diagconf + motscore + 
                                                    motscore2 + scnt1 + swrt1 + 
                                                    sit1 + sdmt1 + CAG + age_0 + 
                                                    CAG:motscore + CAG:age_0,
                         times = c(1, 3, 5))

brier_summary <- extractBrierSummary(pec_obj)

# Print IBS
brier_summary$ibs

# Plot Brier score over time
brier_summary$brier_scores %>%
  ggplot(aes(x = time, y = brier, color = model)) +
  geom_line(size = 1.1) +
  labs(title = "Brier Score Over Time", y = "Brier Score") +
  theme_minimal()
```
