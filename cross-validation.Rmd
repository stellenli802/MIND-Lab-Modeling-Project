...
all_models_data_complete <- ENROLL %>%
  ...
  mutate(
    ...
    CAP = age_0 * (CAG - 34),
    diagconf0 = ifelse(diagconf == 0, 1, 0),
...
for (i in 1:n_splits) {
  ...
  # MRS model
  MRS_model <- coxph(Surv(W, delta) ~ diagconf1 + diagconf2 + diagconf3 +
                       motscore + motscore2 +
...
### Published parameter IPCW-based UNO's C
```{r}
...
### Published parameter figures and tables
```{r}
