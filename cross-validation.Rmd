...
```{r}
# Calculate Uno's C statistic from scratch
# Input: test_data with W, delta, lp (linear predictor); G_hat: KM censoring survival function
#        t_star: time threshold

# Test data and linear predictor
test_data = fitted_models_by_fold[[1]]$test_data
test_data$lp = fitted_models_by_fold[[1]]$lp_CAP

# Training data and KM estimator
train_data <- fitted_models_by_fold[[1]]$train_data
km_cens <- survfit(Surv(W, 1 - delta) ~ 1, data = train_data)

# Time horizon
t_star = 1

# Step function interpolation of KM curve (left-continuous step)
G_hat <- stepfun(km_cens$time, c(1, km_cens$surv), right = FALSE)

n <- nrow(test_data)

# Initialize numerator and denominator of Uno's C
concordant_sum <- 0
weight_sum <- 0
...
for (i in 1:n) {
  for (j in 1:n) {
    Ti <- test_data$W[i]
    Tj <- test_data$W[j]
    di <- test_data$delta[i]
    lpi <- test_data$lp[i]
    lpj <- test_data$lp[j]

    if (Ti < Tj && Ti <= t_star && di == 1) {
      G_Ti <- G_hat(Ti)  # Use a KM prediction function
      wi <- ifelse(di == 1, 1 / G_Ti, 0)

      concordant <- as.numeric(lpi > lpj)
      concordant_sum <- concordant_sum + wi * concordant
      weight_sum <- weight_sum + wi
    }
  }
}

uno_c_hat <- concordant_sum / weight_sum
print(uno_c_hat)
```
...
custom_uno_results <- map_dfr(1:5, function(fold_idx) {
  ...
  bind_rows(
    ...
    group_modify(~ {
      test_data$lp <- .x$lp
      map_dfr(1:5, function(t_star) {
        tibble(
          time = t_star,
          uno_c_custom = compute_uno_c(test_data, G_hat, t_star)
        )
      })
    })
})
